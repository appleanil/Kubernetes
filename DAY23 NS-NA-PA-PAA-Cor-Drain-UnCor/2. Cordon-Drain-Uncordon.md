...
apply one without replicas means simple nginx.

run this command kubectl drain <node-name> --force --ignore-daemonsets --delete-emptydir-data.

we will see that nginx pod will not transfer to another node.
...
apply one replicas: 2 yaml.

riun this command kubectl drain <node-name> --force --ignore-daemonsets --delete-emptydir-data.

we will see all replicas pods transfer to another node.
...
cordon cheysi node again nodeSelector tho pod deploy cheyste appudu pod pending state lo vasthundi and again uncordon cheyste appudu again running state lo vasthundi
